...
# Ressaca de competições continentais (malus leve e condicional)
FATIGUE_CONTINENTAL: int = _get_env_int("FATIGUE_CONTINENTAL", 0)
FATIGUE_DAYS_RAW: str = _get_env_str("FATIGUE_DAYS", "2,3,4")
FATIGUE_DAYS: List[int] = [int(x.strip()) for x in FATIGUE_DAYS_RAW.split(",") if x.strip().isdigit()]
FATIGUE_MALUS_2D: float = _get_env_float("FATIGUE_MALUS_2D", 0.012)
FATIGUE_MALUS_3D: float = _get_env_float("FATIGUE_MALUS_3D", 0.008)
FATIGUE_MALUS_4D: float = _get_env_float("FATIGUE_MALUS_4D", 0.004)
FATIGUE_ELITE_MULT: float = _get_env_float("FATIGUE_ELITE_MULT", 0.60)  # força 4/5 reduz malus
FATIGUE_EXCEPTION_PRESSURE: float = _get_env_float("FATIGUE_EXCEPTION_PRESSURE", 4.5)
# Ligas consideradas "continentais" para a ressaca (padrão: UCL/UEL/UECL + Liberta/Sula)
CONTINENTAL_LEAGUE_IDS_RAW: str = _get_env_str("CONTINENTAL_LEAGUE_IDS", "2,3,4,13,14")
CONTINENTAL_LEAGUE_IDS: set = set([int(x.strip()) for x in CONTINENTAL_LEAGUE_IDS_RAW.split(",") if x.strip().isdigit()])

# Bloqueio de youth/base (U19/U20/U23/Youth/Sub-*)
EXCLUDE_YOUTH: int = _get_env_int("EXCLUDE_YOUTH", 1)
...
DRAW_FORCE3_MIN_ATTACK_GPM: float = _get_env_float("DRAW_FORCE3_MIN_ATTACK_GPM", 1.75)
DRAW_FORCE3_MIN_OPP_DEF_BAD_GPM: float = _get_env_float("DRAW_FORCE3_MIN_OPP_DEF_BAD_GPM", 1.35)
DRAW_FORCE3_MIN_PRESSURE: float = _get_env_float("DRAW_FORCE3_MIN_PRESSURE", 4.0)
DRAW_FORCE3_MIN_CONTEXT: float = _get_env_float("DRAW_FORCE3_MIN_CONTEXT", 0.01)

DRAW_FORCE45_MIN_ATTACK_GPM: float = _get_env_float("DRAW_FORCE45_MIN_ATTACK_GPM", 1.45)
DRAW_FORCE45_MIN_OPP_DEF_BAD_GPM: float = _get_env_float("DRAW_FORCE45_MIN_OPP_DEF_BAD_GPM", 1.25)
DRAW_FORCE45_MIN_PRESSURE: float = _get_env_float("DRAW_FORCE45_MIN_PRESSURE", 3.5)
DRAW_FORCE45_MIN_CONTEXT: float = _get_env_float("DRAW_FORCE45_MIN_CONTEXT", 0.008)
...
# Cache de últimos fixtures por time (para ressaca continental) -> {ts, fixtures:[{ts, league_id}]}
team_last_fixtures_cache: Dict[int, Dict[str, Any]] = {}
...
def _is_youth_text(s: str) -> bool:
    s = (s or "").lower()
    if not s:
        return False
    # padrões comuns: U19/U20/U23, Youth, Sub-19, Sub 20, Juvenil
    tokens = ["u19", "u-19", "u20", "u-20", "u23", "u-23", "youth", "sub-19", "sub 19", "sub-20", "sub 20", "sub-23", "sub 23", "juvenil", "juniores"]
    return any(t in s for t in tokens)

def _is_youth_fixture(league_name: str, home_team: str, away_team: str) -> bool:
    if not EXCLUDE_YOUTH:
        return False
    return _is_youth_text(league_name) or _is_youth_text(home_team) or _is_youth_text(away_team)

async def _fetch_team_last_fixtures_minimal(
    client: httpx.AsyncClient,
    team_id: Optional[int],
    season: Optional[int],
    last_n: int = 5,
) -> List[Dict[str, Any]]:
    """Retorna últimos fixtures FT (mínimo) para ressaca continental.
    Cache em memória por 6h para reduzir chamadas.
    Saída: [{ts:int, league_id:int}, ...]
    """
    if not API_FOOTBALL_KEY or team_id is None or season is None:
        return []
    try:
    ...
    # cache
    cached = team_last_fixtures_cache.get(team_id_int)
    if cached and isinstance(cached, dict):
        ts = cached.get("ts") or 0
        if ts and (_now_utc().timestamp() - float(ts)) < (6 * 3600):
            fx = cached.get("fixtures") or []
            if isinstance(fx, list):
                return fx

    headers = {"x-apisports-key": API_FOOTBALL_KEY}
    params = {"team": team_id_int, "season": season_int, "last": int(last_n), "status": "FT"}
    ...
    try:
        ...
        for it in items:
            try:
                ...
                ts_raw = fixture.get("timestamp")
                lid_raw = league.get("id")
                if ts_raw is None or lid_raw is None:
                    continue
                out.append({"ts": int(ts_raw), "league_id": int(lid_raw)})
            except Exception:
                continue
    except Exception:
        out = []

    team_last_fixtures_cache[team_id_int] = {"ts": _now_utc().timestamp(), "fixtures": out}
    return out

def _fatigue_malus_for_days(days: int) -> float:
    if days == 2:
        return float(FATIGUE_MALUS_2D)
    if days == 3:
        return float(FATIGUE_MALUS_3D)
    if days == 4:
        return float(FATIGUE_MALUS_4D)
    return 0.0

async def _compute_continental_fatigue_malus(
    client: httpx.AsyncClient,
    team_id: Optional[int],
    season: Optional[int],
    kickoff_ts: Optional[int],
    favorite_strength: int,
    pressure_score: float,
    is_favorite_trailing_or_draw: bool,
) -> float:
    """Calcula malus leve por ressaca continental (0..~0.02).
    - só roda se FATIGUE_CONTINENTAL=1
    - reduz malus para força 4/5
    - zera/reduz se pressão muito alta e favorito precisa do gol
    """
    if not FATIGUE_CONTINENTAL:
        return 0.0
    if kickoff_ts is None:
        return 0.0
    if not CONTINENTAL_LEAGUE_IDS:
        return 0.0

    try:
        kickoff_dt = datetime.fromtimestamp(int(kickoff_ts), tz=timezone.utc)
    except Exception:
        return 0.0

    fx = await _fetch_team_last_fixtures_minimal(client, team_id, season, last_n=6)
    if not fx:
        return 0.0
    ...
    for it in fx:
        try:
            ...
            lid = int(it.get("league_id") or 0)
            if lid not in CONTINENTAL_LEAGUE_IDS:
                continue
            dt = datetime.fromtimestamp(ts, tz=timezone.utc)
            delta_days = (kickoff_dt.date() - dt.date()).days
            if delta_days in FATIGUE_DAYS:
                if best_days is None or delta_days < best_days:
    ...
    malus = _fatigue_malus_for_days(int(best_days))

    # Força 4/5: elenco profundo -> reduz
    if favorite_strength >= 4:
        malus *= float(FATIGUE_ELITE_MULT)

    # Se pressão é muito alta e favorito está precisando do gol (empate/derrota): reduz forte (não atrapalhar o faro)
    if is_favorite_trailing_or_draw and pressure_score >= float(FATIGUE_EXCEPTION_PRESSURE):
        malus *= 0.35

    # clamp
    if malus < 0.0:
        malus = 0.0
    return float(min(malus, 0.03))
...
async def _fetch_live_fixtures(client: httpx.AsyncClient) -> List[Dict[str, Any]]:
    ...
    for item in response:
        try:
            ...
            if _is_youth_fixture((league.get("name") or ""), home_team, away_team):
                continue
        ...
        try:
            ...
            for item in response:
                try:
                    ...
                    # Filtra por liga (LEAGUE_IDS) e por base/youth
                    if LEAGUE_IDS and league_id not in LEAGUE_IDS:
                        continue
                    if _is_youth_fixture((league.get("name") or ""), home_team, away_team):
                        continue
...
def _calculate_finishing_efficiency_multiplier(
    total_shots: float,
    total_on: float,
    total_dang: float,
    minute: int,
    ...
    """
    Multiplicador de eficiência ofensiva.

    Objetivo:
    - reduzir "pressão fake"
    - valorizar ataques realmente perigosos
    - suavizar ligas caóticas com baixa conversão
    ...
    try:
        total_shots = float(total_shots or 0.0)
        total_on = float(total_on or 0.0)
        total_dang = float(total_dang or 0.0)
        minute = int(minute or 0)
    except Exception:
        return 1.0, "neutral"

    if total_shots <= 0:
        return 0.92, "no_shots"

    on_target_rate = total_on / max(total_shots, 1.0)

    # Peso leve para ataques perigosos por chute
    danger_per_shot = total_dang / max(total_shots, 1.0)

    efficiency_score = (
        (on_target_rate * 0.75) +
        (min(danger_per_shot / 6.0, 1.0) * 0.25)
    )

    # Ajuste temporal:
    # jogos avançados precisam converter melhor
    if minute >= 75:
        efficiency_score *= 0.94
    elif minute <= 35:
        efficiency_score *= 1.04

    # Multiplicador suave (evita overfit)
    if efficiency_score >= 0.72:
        return 1.20, "elite"
    elif efficiency_score >= 0.55:
        return 1.12, "good"
    elif efficiency_score >= 0.40:
        return 1.00, "neutral"
    elif efficiency_score >= 0.28:
        return 0.92, "weak"

    return 0.84, "fake_pressure"
...
def _calculate_pressure_score_quick(stats: Dict[str, Any]) -> float:
    ...
    efficiency_mult, efficiency_label = _calculate_finishing_efficiency_multiplier(
        total_shots=total_shots,
        total_on=total_on,
        total_dang=total_dang,
        minute=minute,
    )
    ...
    pressure_score *= efficiency_mult
...
def _estimate_prob_and_odd(
    ...
    efficiency_mult, _ = _calculate_finishing_efficiency_multiplier(
        total_shots=total_shots,
        total_on=total_on,
        total_dang=total_dang,
        minute=70,
            ...
            try:
                ...
                metrics = _estimate_prob_and_odd(
                    ...
                    context_boost_prob=context_boost_prob,
                                ...
                                # Ressaca continental (malus leve e condicional)
                fatigue_malus = 0.0
                try:
                    # favorito = lado com maior força (se houver)
                    fav_team_id = fx.get("home_team_id") if favorite_side == "home" else fx.get("away_team_id")
                    fav_needs_goal = bool((fx.get("home_goals")==fx.get("away_goals")) or ((favorite_side=="home" and (fx.get("home_goals") or 0) < (fx.get("away_goals") or 0)) or (favorite_side=="away" and (fx.get("away_goals") or 0) < (fx.get("home_goals") or 0))))
                    fatigue_malus = await _compute_continental_fatigue_malus(
                        client=client,
                        team_id=fav_team_id,
                        season=fx.get("season"),
                        kickoff_ts=fx.get("kickoff_ts"),
                        favorite_strength=int(favorite_strength or 0),
                        pressure_score=float(metrics.get("pressure_score") or 0.0),
                        is_favorite_trailing_or_draw=fav_needs_goal,
                    )
                except Exception:
                    fatigue_malus = 0.0

                if fatigue_malus > 0.0:
                    # ajusta p_final e recalcula odd justa/EV (odd_current segue igual ao modo manual)
                    p0 = float(metrics.get("p_final") or 0.0)
                    p1 = max(0.20, min(0.93, p0 - float(fatigue_malus)))
                    metrics["p_final"] = p1
                    metrics["odd_fair"] = (1.0 / p1) if p1 > 0 else metrics.get("odd_fair")
                    # odd_current neste modo = odd_fair
                    metrics["odd_current"] = metrics["odd_fair"]
                    metrics["ev_pct"] = (p1 * float(metrics["odd_current"]) - 1.0) * 100.0
                    metrics["fatigue_malus"] = float(fatigue_malus)
...
# CORTE POR GOLEADA / CONTEXTO / PERFIL UNDER/OVER
                ...
                # CORREÇÃO REFINADA: empates estavam excessivamente travados
                # Objetivo: aliviar travas em empates "vivos" sem abrir torneira para jogos mortos
                is_draw = (score_diff == 0)
                if is_draw:

                    total_goals = home_goals + away_goals
                    is_zero_zero = (home_goals == 0 and away_goals == 0)
                    is_one_one = (home_goals == 1 and away_goals == 1)
                    is_chaotic_draw = (total_goals >= 4)

                    # Empates continuam exigindo algum nível mínimo de força/contexto
                    # mas sem bloquear automaticamente força 3
                    if fav_strength <= 1:
                        block_counters["draw_filter"] += 1
                        continue

                    # Super-under ainda protegido
                    is_super_under_draw = _is_match_super_under(
                        attack_home_gpm,
                        defense_home_gpm,
                        attack_away_gpm,
                        defense_away_gpm
                    ...
                    if is_super_under_draw:
                        ...
                        # Empates mais vivos recebem leve flexibilização
                        relaxed_pressure = DRAW_BOTH_WEAK_ATTACK_ALLOW_PRESSURE
                        relaxed_context = DRAW_BOTH_WEAK_ATTACK_ALLOW_CONTEXT

                        if is_one_one:
                            relaxed_pressure -= 0.6
                            relaxed_context -= 0.003

                        allow_super_under = (
                            fav_strength >= 3
                            and (pressure_score >= relaxed_pressure)
                            and (context_boost_prob >= relaxed_context)
                            and (minute_int >= 48)
                        )
                    ...
                    if fav_side == "home":
                        ...
                        # Em empate quente, permitir ausência de favorito claro
                        hot_draw = (
                            pressure_score >= 6.5
                            and context_boost_prob >= 0.010
                            and minute_int >= 60
                        )
                        ...
                        fav_attack_gpm = max(attack_home_gpm, attack_away_gpm)
                        opp_defense_gpm = max(defense_home_gpm, defense_away_gpm)
                        facing_defense_draw = opp_defense_gpm

                    # Defesa forte continua protegida, porém mais flexível em 1-1
                    ...
                    if opp_def_strong_draw and fav_strength < STRONG_DEF_FAV_MIN_STRENGTH:
                        ...
                        strong_def_escape = (
                            is_one_one
                            and pressure_score >= 6.0
                            and minute_int >= 58
                        )

                        if not strong_def_escape:
                            block_counters["strong_defense_favorite_draw"] += 1
                            continue
                    ...
                    if fav_strength == 3:
                        ...
                        min_pressure = DRAW_FORCE3_MIN_PRESSURE
                        min_context = DRAW_FORCE3_MIN_CONTEXT
                    else:
                        min_attack = DRAW_FORCE45_MIN_ATTACK_GPM
                        min_opp_def = DRAW_FORCE45_MIN_OPP_DEF_BAD_GPM
                        min_pressure = DRAW_FORCE45_MIN_PRESSURE
                        min_context = DRAW_FORCE45_MIN_CONTEXT

                    # Ajustes inteligentes por tipo de empate
                    if is_zero_zero:
                        # 0-0 ainda precisa de pressão forte
                        min_pressure += 0.7
                        min_context += 0.003

                    elif is_one_one:
                        # 1-1 historicamente é excelente para over tardio
                        min_pressure -= 0.8
                        min_context -= 0.004
                        min_opp_def -= 0.08

                    elif is_chaotic_draw:
                        # 2-2, 3-3 etc.
                        min_pressure -= 1.0
                        min_context -= 0.005
                        min_opp_def -= 0.10

                    # Final de jogo: aliviar um pouco sem perder qualidade
                    if minute_int >= 70:
                        min_pressure -= 0.4
                        min_context -= 0.002
                    ...
                    # Em empates vivos, não exigir defesa tão ruim explicitamente
                    defense_gate_failed = (opp_defense_gpm < min_opp_def)

                    if defense_gate_failed:
                        ...
                        escape_live_draw = (
                            ...
                            and pressure_score >= (min_pressure + 0.6)
                            and context_boost_prob >= (min_context + 0.003)
                        )

                        if not escape_live_draw:
                            block_counters["draw_filter"] += 1
                            continue

                    # Dinâmica ao vivo
                    allow_draw = (
                        pressure_score >= min_pressure
                        and context_boost_prob >= min_context
                        and minute_int >= 48
                    )
                    ...
                    allow_elite = (
                        ...
                        and pressure_score >= (DRAW_ELITE_MIN_PRESSURE - 0.5)
                        and context_boost_prob >= (DRAW_ELITE_MIN_CONTEXT_BOOST - 0.002)
                    )

                    # Empate caótico pode escapar com dinâmica extremamente forte
                    allow_chaos = (
                        is_chaotic_draw
                    ...
                    if not (allow_draw or allow_elite or allow_chaos):
                        block_counters["draw_filter"] += 1
                        continue
