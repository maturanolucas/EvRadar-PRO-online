...
def _estimate_prob_and_odd(
    ...
    # Forma recente ofensiva
    if fav_strength >= 4:
        recent_attack_bonus += 0.012
    elif fav_strength >= 3:
        recent_attack_bonus += 0.008
    elif fav_strength >= 2:
        recent_attack_bonus += 0.004

    # Defesa vulnerável do adversário
    if opp_defense_gpm >= 1.80:
        recent_defense_bonus += 0.012
    elif opp_defense_gpm >= 1.45:
        recent_defense_bonus += 0.008
    elif opp_defense_gpm >= 1.20:
        recent_defense_bonus += 0.004

    # Caos produtivo
    if fav_attack_gpm >= 1.60 and opp_defense_gpm >= 1.40:
        chaos_bonus += 0.010

    # Favorito precisando buscar resultado
    if score_home == score_away:
        need_bonus += 0.004

    context_boost_prob += (
        recent_attack_bonus
        + recent_defense_bonus
        + chaos_bonus
        + need_bonus
    )
    ...
    # ============================================
    # HUMAN ENTRY TRIGGER - LUCAS STYLE
    # ============================================

    human_trigger_bonus = 0.0

    # Favorito empatando ou perdendo em casa
    if (
        score_home == score_away
        and fav_strength >= 3
    ):
        human_trigger_bonus += 0.006

    # Zebra marcou cedo -> jogo abre
    if (
        minute_int <= 70
        and (
            (favorite_side == "home" and away_goals > home_goals)
            or
            (favorite_side == "away" and home_goals > away_goals)
        )
    ):
        human_trigger_bonus += 0.012

    # Ataque forte vs defesa vulnerável
    if (
        fav_attack_gpm >= 1.70
        and opp_defense_gpm >= 1.45
    ):
        human_trigger_bonus += 0.010

    # Caos produtivo
    if (
        home_defense_gpm >= 1.40
        and away_defense_gpm >= 1.40
    ):
        human_trigger_bonus += CHAOS_PRODUCTIVE_BONUS

    context_boost_prob += human_trigger_bonus
