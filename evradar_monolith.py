...
# 5) Se quem está perdendo tem "pouca munição"
                ...
                if api_odd is not None and api_odd < MIN_ODD:
                    ...
                    continue
                # Força 0 (sem favorito): em linha alta, exige munição real de quem está perdendo
                try:
                    fav_strength0 = int(fx.get('favorite_strength') or 0) == 0
                except Exception:
                    fav_strength0 = True
                if fav_strength0:
                    ...
                    line_now = (hg + ag + 0.5)
                    if line_now >= HIGH_LINE_START:
                        trailing_attack = None
                        if hg < ag:
                            trailing_attack = attack_home_gpm
                        elif ag < hg:
                            trailing_attack = attack_away_gpm
                        if trailing_attack is not None and trailing_attack < BALANCED_HIGHLINE_TRAIL_MIN_ATTACK_GPM:
                            block_counters['balanced_trailer_weak'] += 1
                            continue
                        if metrics.get('pressure_score', 0.0) < (MIN_PRESSURE_SCORE + BALANCED_HIGHLINE_EXTRA_PRESSURE):
                            block_counters['balanced_trailer_weak'] += 1
                            continue
...
# ========= Layout objetivo (sempre) =========
def _format_simple_alert_text(fixture: Dict[str, Any], linha_str: str) -> str:
    jogo = "{home} vs {away} — {league}".format(
        home=fixture["home_team"],
        away=fixture["away_team"],
        league=fixture["league_name"],
    )
    minuto = fixture.get("minute", "?")
...
def _format_alert_text(fixture: Dict[str, Any], metrics: Dict[str, float]) -> str:
    total_goals = int(fixture.get("home_goals", 0) or 0) + int(fixture.get("away_goals", 0) or 0)
    linha_str = "Over {v:.1f}".format(v=(total_goals + 0.5))
    return _format_simple_alert_text(fixture, linha_str)

def _format_watch_text(fixture: Dict[str, Any], metrics: Dict[str, float]) -> str:
    return _format_alert_text(fixture, metrics)

def _format_manual_no_odds_text(fixture: Dict[str, Any], metrics: Dict[str, float]) -> str:
    return _format_alert_text(fixture, metrics)

def _format_pattern_only_text(fixture: Dict[str, Any], metrics: Dict[str, float]) -> str:
    return _format_alert_text(fixture, metrics)
