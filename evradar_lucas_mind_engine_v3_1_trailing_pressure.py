...
def _stats_for_trailing_team_pressure(
    ...
    """
    V3.1 Lucas: quando existe um time perdendo, o Live/Pressure Score usa
    exclusivamente as ações ofensivas desse time.

    - Casa perdendo  -> mantém home_* e zera away_*
    - Fora perdendo  -> mantém away_* e zera home_*
    - Empate         -> mantém os dois lados (não existe perdedor definido)

    Metadados não ligados a um lado são preservados para auditoria.
    """
    try:
    ...
    if hg == ag:
        out = dict(stats)
        out["pressure_side"] = "both_draw"
        return out

    trailing_prefix = "home_" if hg < ag else "away_"
    leading_prefix = "away_" if trailing_prefix == "home_" else "home_"

    out: Dict[str, Any] = {}
    for k, v in stats.items():
        if k.startswith(leading_prefix):
            # Zera o líder para que _sum_stat continue compatível e passe a
            # representar somente a produção de quem precisa buscar o placar.
            out[k] = 0
        else:
            out[k] = v

    out["pressure_side"] = "home_trailing" if trailing_prefix == "home_" else "away_trailing"
    return out
    ...
    async with httpx.AsyncClient() as client:
        ...
        for fx in fixtures:
            try:
                ...
                # V3.1 Lucas: se há um perdedor, a pressão do 2ºT é medida SOMENTE
                # pelas ações dele. Em empate, mantém a leitura bilateral atual.
