...
def _second_half_stats_view(fixture_id: int, minute: int, stats: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Se ele só ligou tarde e não existe baseline, usa o acumulado como fallback para
    não matar alertas, mas marca second_half_mode='fallback_full_match'.
    """
    ...
    # Se não há baseline porque o robô entrou tarde, mantém funcionalidade com fallback.
    if baseline is None:
        out = dict(stats)
        out["second_half_mode"] = "fallback_full_match"
        return out
...
def _second_half_label(mode: Any) -> str:
    ...
    if mode_str == "baseline_created":
        return "2ºT baseline"
    if mode_str == "fallback_full_match":
        return "jogo todo fallback"
